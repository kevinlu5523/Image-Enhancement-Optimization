...
1. **影像前置判斷**：系統讀入原始影像後，首先計算其平均亮度值 `τ`。
2. **自適應分流**：依據 `τ` 值的臨界點進行條件判定：
   * **Yes (白天模式，`τ > 40`)**：套用 Log-Gamma 公式，並以 **PCQI** 為優化指標。
   * **No (夜間模式，`τ <= 40`)**：套用 Log-Gamma 公式，並以 **BRISQUE** 為優化指標。
3. **MPA 參數迭代最佳化**：進入海洋掠食者演算法的優化迴圈：
   * 在每次迭代中，利用 MPA 動態尋找最佳的 `a` 與 `b` 參數組合。
   * 檢查當前迭代次數是否達到最大值 (`i = max_iter`)。若尚未達到 (No)，則返回 Log-Gamma 繼續進行更新與計算。

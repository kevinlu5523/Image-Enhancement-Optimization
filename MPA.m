...
outputBasePath = fullfile('dataset', 'MPA'); % 輸出路徑
...
for imgIdx = 1:numImages
    % 顯示當前處理進度
    disp(sprintf('正在處理第 %d/%d 張影像', imgIdx, numImages));
    ...
    for iter = 1:Max_iter
        ...
        % 顯示當前迭代資訊
        disp(['影像 ', baseName, ' - 迭代 ', num2str(iter), ': a = ', num2str(gather(Elite(1))), ...
    ...
    % 顯示最佳參數和 PCQI 分數
    disp(['影像 ', baseName, ' - MPA 優化結果（公式 3）：']);
